/**
 * @file net.h
 * @brief Core network stack definitions and interfaces
 */
...
/* Interface casting macro */
#define NET_IFACE(x) ((struct net_iface *)(x))

/* Protocol types (same as Ethernet types) */
...
struct net_device {
    ...
    struct net_iface *ifaces; /* NOTE: protect ifaces with mutex after net_run() */
    unsigned int index;
    char name[IFNAMSIZ];
    uint16_t type;
    uint16_t mtu;
