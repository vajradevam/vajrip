/**
 * @file tcp_server.c
 * @brief TCP echo server using custom TCP/IP stack
 * 
 * Simple TCP server that accepts connections and echoes back received data.
 * Demonstrates socket listening, acceptance, and bidirectional communication.
 */
...
/**
 * @brief Main application entry point for TCP server
...
{
    int soc, acc;                    /* Socket descriptors */
    ...
    /* ========================================================================
     * Application Logic - TCP Echo Server
     * ======================================================================== */
    ...
    infof("connection accepted, foreign=%s", 
    ...
    while (!terminate) {
        ...
        if (ret == 0) {
            ...
            debugf("connection closed by client");
            break;
        }
    ...
    /* Cleanup: close connection and listening sockets */
    sock_close(acc);
    sock_close(soc);
